...
```text
https://github.com/runceel/SkimDownForGitHubCopilotApp/tree/v1.1.2/.github/extensions/skimdown
```
...
- 「作った 3 本のドキュメントを SkimDown でこの順に見せて」（ドキュメントセットとして提示します）
...
| `show_markdown_set` | `documents`, `title?`, `id?` | 読んでほしい Markdown を順序付きのセットとして提示します。ファイルとインライン Markdown を混在できます |
...
`open_canvas` の入力（`path` / `markdown` / `documents` / `title` / `id` / `source`）で初期表示を指定できます。

### ドキュメントセット

`show_markdown_set` は「今回の成果物を、この順で読んでほしい」を伝えるためのアクションです。

- `documents` は最大 25 件の配列で、各項目は `path`（ディスク上の Markdown）か
  `markdown`（インライン本文）のどちらかに、`title?` と `description?` を添えます。
- セットは「このセッション」ソースの**先頭グループ**として、指定した順に並びます。
  他のグループには重複表示しません。
- 提示するとソースが「このセッション」へ切り替わり、先頭のドキュメントが開きます。
- セッションが持てるセットは常に 1 つです。新しいセットは前のセットを置き換えます。
- 見つからないファイルや Markdown 以外の項目はセットから除外し、残りを表示します。
  除外した項目はアクションの戻り値の `skipped` で報告します。
...
| ソース | 内容 |
| --- | --- |
| **このセッション** | ドキュメントセット（提示されている場合）を先頭に、セッション成果物（`plan.md` と `files/**`）、エージェントが編集した `.md`、`show_markdown` で渡されたドキュメント、チェックポイントを、グループ見出し付きで表示します。セット以外は新しい順です |
...
| サイドバー開閉 | `Ctrl+B` |
| ドキュメントセットの移動 | `Ctrl+PageUp` / `Ctrl+PageDown`、ツールバーの `‹ n/N ›` |
...
- ドキュメントセットのナビゲーションは、セットが提示されているときだけツールバーに出ます。端で止まり、先頭や末尾を越えて回り込みません。
