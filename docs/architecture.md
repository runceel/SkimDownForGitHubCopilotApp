...
ユーザーが指定した既存の Markdown である。読み手が明示的に求めたときに限り、
読書対象と引用を添えた指示をホストセッションへ引き渡す。

次の責務は持たない。

- Markdown の編集や、リポジトリ内コンテンツの暗黙的な変更
- ユーザー操作を伴わないセッションへの働きかけ
...
flowchart TB
    ...
    provider --> asset
    provider <--> state
    provider -.->|"ユーザー操作による user turn"| host
...
| コンポーネント | 責務 | 境界 |
| --- | --- | --- |
| 拡張プロバイダ | canvas の登録、インスタンスのライフサイクル、ホストからのアクション受付、読み手の指示のセッションへの引き渡し | UI の描画や Markdown の解釈を持たない |
...
| ドキュメントセット | セッション内で安定したセット ID | 次のセットが提示されるまで（常に 1 つ） |
...
- **canvas host と拡張プロバイダ**は、canvas の open、close、action 契約で通信する。
  加えて、読み手が canvas 上で明示的に要求したときだけ、拡張プロバイダは読書対象と引用を
  添えた user turn をホストセッションへ 1 件送る。この経路は外向きであり、ユーザー操作を
  伴わない起点から使わない。
...
| 実行中セッション | インライン文書、ドキュメントセット、セッション中に扱った文書、最後の選択 | 拡張プロセスのメモリ。既定では永続化しない |
...
事後確認できるようにする。また、診断へ Markdown 本文、URL、user agent、workspace/session
情報を記録しない。

可観測性は、それ自体が停止の原因になってはならない。ホストのログ経路は JSON-RPC 要求であり、
呼び出し時の例外としても、後から reject する promise としても失敗し得る。拡張プロセスは
canvas の実体なので、この種の失敗がプロセスを終わらせると読書 UI ごと失われる。診断処理と
ログ送出の失敗、および await されない処理の失敗は、記録したうえで実行を継続する。
例外は起動で、セッションへ参加できない場合は隠さずホストへ委ねる（[ADR 0014](adr/0014-keep-the-reader-alive-through-observability-failures.md)）。
...
4. 拡張プロセスの stdout は JSON-RPC 専用である。拡張プロセスで `console.log` を使わず、
   ホストのログ経路または診断成果物を使う。ログ送出は呼び出し側から見て失敗しない。
   ホストが受理する severity へ正規化し、同期例外と promise の reject の両方を送出側で
   吸収する。セッション参加後の未処理 reject と未捕捉例外はプロセス境界で受け止め、
   記録したうえで継続する。セッションへ参加できない場合だけは隠さず非ゼロ終了する。
...
17. セッションへの user turn 送信は、canvas 上のユーザーの明示操作からのみ開始する。
    ファイル監視、イベント配信、ホストからの canvas アクション、再接続処理を起点にしない。
18. 送信対象の文書同一性は拡張プロセス側の選択状態を正とし、クライアントが送るパスや
    文書識別子をそのまま送信対象として信頼しない。
19. 送信する文面へ、拡張がユーザーの意図を足さない。加えてよいのは要求の出所と読書対象の
    提示に限る。質問文と引用には長さ上限を、送信には受付頻度の上限を課す。
20. 質問文と引用本文を診断やログへ記録しない。
21. ドキュメントセットは提示順を表す参照の並びであり、本文の第 2 の保管場所ではない。
    インライン項目の本文は既存のインライン文書として保持し、その上限、永続化、消去、
    リモートコンテンツ同一性の経路を共有する。セット内の移動は方向だけを受け取り、
    移動先は拡張プロセス側の一覧と選択状態から解決する。
