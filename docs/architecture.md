...
- **アセット origin** は、拡張が管理するシェル、renderer、ベンダー資産、状態 API、
  イベントストリームを提供する。ここは信頼されたアプリケーションコードの領域であり、
  API とイベントストリームはインスタンス固有の capability を持つ同一 origin の要求だけを受け付ける。
...
診断経路を通じて拡張へ返す。この経路はインスタンス固有の capability で認証し、
...
11. アセット origin の API と SSE は、インスタンス capability と同一 origin のブラウザー要求を
    必須にする。loopback bind や port 番号だけを認証として扱わない。
12. ローカルファイル操作は、workspace、セッションで登録された文書、または明示的に承認された
    root の内側に限定し、クライアントが送るパスをそのまま信頼しない。
13. 診断 API は schema、body サイズ、受付頻度、保存総量の境界を外さない。
