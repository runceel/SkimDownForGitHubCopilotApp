...
- ログ送出は呼び出し側から見て失敗しない。ホストが受理する severity へ正規化し、同期例外と promise の reject の両方を吸収する。セッション参加後の未処理 reject と未捕捉例外は記録したうえで継続し、拡張プロセスを終了させない。セッションへ参加できない場合だけは隠さず非ゼロ終了する。
